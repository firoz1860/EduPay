...
export async function create(req: Request, res: Response): Promise<Response> {
  const result = await service.createUser(req.body, getActor(req), req.requestId);
  return created(res, result, 'User created');
}

export async function update(req: Request, res: Response): Promise<Response> {
  const result = await service.updateUser(req.params.id, req.body, getActor(req), req.requestId);
...
export async function list(req: Request, res: Response): Promise<Response> {
  const { data, meta } = await service.listUsers(req.query as never);
